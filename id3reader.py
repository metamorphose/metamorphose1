...
""" Read ID3 tags from a file.
    ...
    Modifications to work with Métamorphose:
    Added _() gettext for translations.
    - ianaré
"""

__version__ = '1.51.20040913'    # History at the end of the file.
...
#_encodings = ['iso8859-15', 'utf-16', 'utf-16be', 'utf-8', 'iso-8859-5']
...
_simpleDataMapping = {
    _(u"album"):        ('TALB', 'TAL', 'v1album'),
    _(u"performer"):    ('TPE1', 'TP1', 'v1performer'),
    _(u"title"):        ('TIT2', 'TT2', 'v1title'),
    _(u"track"):        ('TRCK', 'TRK', 'v1track'),
    _(u"year"):         ('TYER', 'TYE', 'v1year'),
    _(u"genre"):        ('TCON', 'TCO', 'v1genre'),
}
    ...
    def _interpret(self):
        ...
        if self.id[0] == 'T':
            ...
            if self.rawData[0] == 'z':
                ...
                raise Id3Error, _("Unknown CDM compression: %02x") % self.rawData[0]
...
class Reader:
    ...
    def __init__(self, file):
        ...
        if isinstance(self.file, type(u'')):
    ...
    def _readBytes(self, num, desc=''):
        ...
        if num > self.bytesLeft:
            ...
            raise Id3Error, _("Long read (%s): (%d > %d)") % (desc, num, self.bytesLeft)
        ...
        if len(bytes) < num:
            ...
            raise Id3Error, _("Short read (%s): (%d < %d)") % (desc, len(bytes), num)
    ...
    def _readId3(self):
        ...
        if self.header.majorVersion == 2:
            ...
            raise Id3Error, _("Unsupported major version: %d") % self.header.majorVersion
        ...
        if self.header.majorVersion >= 3:
            if self.header.flags & 0x40:
                ...
                if self.header.majorVersion == 3:
                    ...
                    self._readExtHeader = _readExtHeader_rev4
    ...
    def _readExtHeader_rev4(self):
        ...
        self._readBytes(size, 'rev4ehdata')
    ...
    def getValue(self, id, encoding):
        """ Return the value for an ID3 tag id, or for a
            ...
            or return empty unicode string if there is no such value.
        """
        if self.frames.has_key(id):
            if hasattr(self.frames[id], 'value'):
                if isinstance(self.frames[id].value, unicode):
                    ...
                    # to fix item being returned as list:
                    except TypeError:
                        # only get the last value of the first 'letter', as there could be lead chars:
                        returnValue = self.frames[id].value[0][-1].decode(encoding)
                        for letter in self.frames[id].value[1:]:
                            returnValue += unicode(letter, encoding)
                        pass
                return returnValue
